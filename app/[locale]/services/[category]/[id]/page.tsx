...
import { Star, MapPin, MessageCircle, Phone, Briefcase } from "lucide-react"
...
export default async function ProviderProfilePage({ params }: Props) {
  ...
  const catName = locale === 'ar' ? provider.category?.nameAr : locale === 'fr' ? provider.category?.nameFr : provider.category?.name
  const bio = locale === 'ar' ? (provider.bioAr ?? provider.bio) : locale === 'fr' ? (provider.bioFr ?? provider.bio) : provider.bio

  const availabilityColor = provider.availability === 'AVAILABLE' ? 'bg-green-100 text-green-700' : provider.availability === 'BUSY' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'
  const availabilityLabel = provider.availability === 'AVAILABLE'
        ...
        <Card className="mb-6">
          <CardContent className="p-6">
            <div className="flex flex-col sm:flex-row items-start gap-5">
              <div className="w-24 h-24 rounded-full bg-red-100 flex items-center justify-center text-4xl font-bold text-red-700 flex-shrink-0 overflow-hidden">
                {provider.avatarUrl
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  {provider.isVerified && <Badge className="bg-green-100 text-green-700">✓ {locale === 'fr' ? 'Vérifié' : locale === 'ar' ? 'موثوق' : 'Verified'}</Badge>}
                ...
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <span className="text-sm text-gray-500 ml-1">({provider.reviewCount} {locale === 'fr' ? 'avis' : locale === 'ar' ? 'تقييم' : 'reviews'})</span>
                </div>
                ...
                {provider.yearsExperience && (
                  <div className="flex items-center gap-1 text-sm text-gray-500">
                    <Briefcase className="h-3 w-3" />{provider.yearsExperience} {locale === 'fr' ? "ans d'expérience" : locale === 'ar' ? 'سنوات خبرة' : 'years experience'}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-5">
              ...
              <Button variant="outline" className="flex-1" asChild>
                <a href={`tel:${provider.phone}`}>
                  <Phone className="h-4 w-4 mr-2" />{locale === 'fr' ? 'Appeler' : locale === 'ar' ? 'اتصل' : 'Call'}
                </a>
              </Button>
            </div>
          </CardContent>
          ...
          <Card className="mb-6">
            <CardContent className="p-6">
              <h2 className="font-semibold text-lg mb-3 text-gray-800">
                {locale === 'fr' ? 'À propos' : locale === 'ar' ? 'نبذة' : 'About'}
              </h2>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {provider.portfolioImages.map(img => (
                  <div key={img.id} className="aspect-square rounded overflow-hidden bg-gray-100">
